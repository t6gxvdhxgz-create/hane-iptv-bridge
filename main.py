#!/usr/bin/env python3
"""
HaNe IPTV Proxy v4.1.2 - Updated for better IPTV panel compatibility
"""
...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def _handle_proxy(self, query: Mapping[str, List[str]], head_only: bool) -> None:
        ...
        # Forward real client IP - this is the main fix for 456 errors
        forwarded_for = self.headers.get("X-Forwarded-For", "").split(",")[0].strip()
        real_ip = forwarded_for or (self.client_address[0] if hasattr(self, "client_address") else "")
        if real_ip:
            request.add_header("X-Forwarded-For", real_ip)
            request.add_header("X-Real-IP", real_ip)
            request.add_header("X-Client-IP", real_ip)
        
        request.add_header("Accept-Encoding", "identity")
        
        # User-Agent
        if not request.has_header("User-Agent"):
            request.add_header("User-Agent", self.headers.get("User-Agent", f"HaNeIPTV/{VERSION}"))
        
        # Allow override via environment variable
        custom_ua = os.environ.get("UPSTREAM_USER_AGENT", "").strip()
        if custom_ua:
            request.add_header("User-Agent", custom_ua)
